        ...
        <div key={data?.id} className="flex">
            <Link to={`/watch?v=${data?.id}`}>
                <div className={(isSidebarOpen) ? 'md:w-96' : 'md:w-[28rem]'}>
                    ...
                    <p className="text-l px-2 font-semibold">{data?.snippet.title}</p>
                    <p className="text-sm pt-1 px-2 font-sans">{data?.snippet.channelTitle}</p>
                    <p className="text-sm pt-1 px-2 font-sans">{formatNumber(data?.statistics.viewCount, 1)} views</p>
                </div>
            </Link>
        </div>
    )
